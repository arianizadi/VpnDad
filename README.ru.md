...
Для профилей MasterDnsVPN рекомендуется режим `nativePacket` с
`PROTOCOL_TYPE` = `TCP`. Используйте `hevSocks` только как fallback, если
разработчик специально просит legacy SOCKS bridge path.
...
`arianizadi/MasterDnsVPN` для этого package и mobile bridge diagnostics. FEC
support отдельный, только для custom build, и для normal testing должен
...
`masterdns.clientConfig` использует те же uppercase keys, что и desktop
client JSON/TOML config MasterDnsVPN. Legacy fields вроде `encryptionKey`,
`encryptionMethod`, `encryptionLevel`, `baseEncodeData` и FEC shortcuts все еще
import поддерживают; отсутствующие canonical keys заполняются из этих aliases.

`runtimeMode` управляет iOS transport:

- `nativePacket`: рекомендуется для normal MasterDnsVPN testing. Требует
  `PROTOCOL_TYPE` `TCP` и `LOCAL_DNS_ENABLED` `true`. Tunnel extension подает
  IP packets из `NEPacketTunnelFlow` в Go gVisor netstack adapter. TCP flows и
  DNS UDP/53 идут через MasterDnsVPN stream и DNS-cache paths; generic non-DNS
  UDP не передается.
- `hevSocks`: legacy fallback. Требует `PROTOCOL_TYPE` `SOCKS5` и
  `LOCAL_DNS_ENABLED` `false`.

`DATA_ENCRYPTION_METHOD` следует desktop bounds `0` through `5`. Editor
предупреждает о legacy methods ниже `3`; AES-GCM methods `3`, `4` и `5`
остаются recommended choices для iOS testing.
...
Сейчас VpnDad имеет два MasterDnsVPN runtime modes:

- Рекомендуется: `nativePacket` с `PROTOCOL_TYPE` `TCP`.
...
- `nativePacket`: iOS Packet Tunnel -> Go gVisor packet adapter ->
  MasterDnsVPN runtime без local listener sockets.

Поддерживается сейчас:

- Profiles с `protocol: "masterdns"`.
- Full desktop client config storage под `masterdns.clientConfig`.
- Legacy MasterDnsVPN aliases импортируются в canonical `clientConfig` keys.
- SOCKS5 server mode через mobile bridge в `hevSocks` runtime mode.
- Native packet mode для MasterDnsVPN TCP flows и DNS UDP/53.
- Desktop encryption method bounds `0` through `5`, с iOS warnings для legacy
  non-AES-GCM methods.
- `BASE_ENCODE_DATA`, compression, MTU, resolver strategy, duplication,
  worker/timer, ARQ, FEC и logging fields в editor.
- Multiple domains и UDP IP/CIDR resolvers в imported JSON и editor.
- Keychain-only MasterDnsVPN secret persistence и secret-free profile export.
- Custom-fork FEC fields, experimental и collapsed по умолчанию в editor.
